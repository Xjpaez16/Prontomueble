                    ...
                    <div class="card-body">
                        <form action="Catalogo?menu=Proveedor" method="POST">
                            <div class="mb-3">
                                ...
                                <input type="text" value="${proveedor.getId()}" name="txtId" class="form-control" placeholder="Ingrese ID" required ${not empty proveedor.getId() ? 'readonly' : ''}>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Nombre</label>
                                <input type="text" value="${proveedor.getNombre()}" name="txtNombre" class="form-control" placeholder="Ingrese nombre" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Dirección</label>
                                <input type="text" value="${proveedor.getDireccion()}" name="txtDireccion" class="form-control" placeholder="Ingrese dirección" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Persona de Contacto</label>
                                <input type="text" value="${proveedor.getP_contacto()}" name="txtContacto" class="form-control" placeholder="Ingrese persona de contacto" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Telefonos</label>

                              
                                <input type="text" value="${proveedor.getTelefonos().size() > 0 ? proveedor.getTelefonos().get(0).getnTelefono() : ''}" 
                                       name="txtTelefono" class="form-control mb-2" placeholder="Ingrese teléfono" required>

                        
                                <input type="text" value="${proveedor.getTelefonos().size() > 1 ? proveedor.getTelefonos().get(1).getnTelefono() : ''}" 
                                            ...
                                            <table class="table" style="width: 100%" id="clientesTable">
                                                ...
                                                <c:forEach var="proveedor" items="${listaProveedores}">
                                                    <tr>
                                                        ...
                                                        <td>${proveedor.getP_contacto()}</td>
                                                  
                                                        <td>
                                                    <c:forEach var="telefono" items="${proveedor.getTelefonos()}">
                                                        ${telefono.getnTelefono()}<br>
                                                    </c:forEach>
                                                    </td>
                                                    <td>
                                                        <a class="Btnedit" href="Catalogo?menu=Proveedor&accion=Editar&id=${proveedor.getId()}">Editar
                                                        ...
                                                        <a class="button" type="button" href="Catalogo?menu=Proveedor&accion=Delete&id=${proveedor.getId()}"><span class="button__text">Delete</span>
